...
import StatusAdvanceBalcon from "../status/[id]";
...
const EditFactureAdvance = () => {
  ...
  const onClickSo = (data: any) => {
    fetch("/api/mailBalcon/mailAdvaSoli", {
      method: "POST",
      headers: {
  ...
  const onClickTe = (data: any) => {
    fetch("/api/mailBalcon/mailAdvaTeso", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const onClickFi = (data: any) => {
    fetch("/api/mailBalcon/mailAdvaFinan", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const onClickTes = (data: any) => {
    fetch("/api/mailBalcon/mailAdvaTesor", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const printAdvanceBalcon = () => {
    if (Router.asPath !== Router.route) {
      const advanceBalconId = Router.query.id as string;
      Router.push({ pathname: "/advanceBalcon/print/" + advanceBalconId });
    } else {
      setTimeout(printAdvanceBalcon, 1000);
    }
  };
  ...
  const txtBGR = () => {
    if (Router.asPath !== Router.route) {
      const advanceBId = Router.query.id as string;
      Router.push({ pathname: "/advanceBalcon/txtBGR/" + advanceBId });
    } else {
  ...
  const buttons = {
    edit: (rowData: FactureAdvanceBalcon) => {
      if (
        ...
        CheckFinished(auth, [7], formik.values?.advanceState, Cerrado)
      )
        return;
  ...
  return (
    <>
      <title>Balcon | Editar Anticipo</title>
