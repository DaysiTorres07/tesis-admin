...
import { AdvanceBalcon, ModalProps, ResponseData } from "../../../lib/types";
import HttpClient from "../../../lib/utils/http_client";

interface Props extends ModalProps<AdvanceBalcon> {
  initialData?: AdvanceBalcon;
}

const StatusAdvanceBalcon = (props: Props) => {
  const { auth } = useAuth();
  const [initialValues, setInitialValues] = useState<AdvanceBalcon>(null);

  const loadData = async () => {
    const solicitudeId = Router.query.id as string;
    const response: ResponseData = await HttpClient(
      "/api/advanceBalcon/" + solicitudeId,
      "GET",
      auth.userName,
      auth.role
    );
...
export default StatusAdvanceBalcon;
