...
import { ModalProps, ResponseData, SolicitudeBalcon } from "../../../lib/types";
import HttpClient from "../../../lib/utils/http_client";
import { useFormik } from "formik";
import theme from "../../../lib/styles/theme";

interface Props extends ModalProps<SolicitudeBalcon> {
  initialData?: SolicitudeBalcon;
}

const StatusSolicitudeBalcon = (props: Props) => {
  const { auth } = useAuth();
  const [initialValues, setInitialValues] = useState<SolicitudeBalcon>(null);

  const loadData = async () => {
    const solicitudeId = Router.query.id as string;
    const response: ResponseData = await HttpClient(
      "/api/solicitudeBalcon/" + solicitudeId,
      "GET",
      auth.userName,
      auth.role
    );
...
export default StatusSolicitudeBalcon;
