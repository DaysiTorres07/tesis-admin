...
import StatusSolicitudeBalcon from "../status/[id]";
...
const EditFacture = () => {
  ...
  const [initialValues, setInitialValues] = useState<SolicitudeBalcon>({
    number: 0,
    soliciter: "",
    date: FormatedDate(),
    details: "",
  ...
  const onSubmit = async (formData: SolicitudeBalcon) => {
    if (Router.asPath !== Router.route) {
      setLoading(true);
      const solicitudeBalconId = Router.query.id as string;
      const factureBalconItems = await UploadSolicitudeImages(items);
      const requestData = {
        ...formData,
        itemsComment,
        items: factureBalconItems,
        id: solicitudeBalconId,
      };
      const response: ResponseData = await HttpClient(
  ...
  const onClickCo = (data: any) => {
    fetch("/api/mailBalcon/mailConta", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const onClickFi = (data: any) => {
    fetch("/api/mailBalcon/mailFinan", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const printSolicitudeBalcon = () => {
    if (Router.asPath !== Router.route) {
      const solicitudeBalconId = Router.query.id as string;
      Router.push({
        pathname: "/solicitudeBalcon/print/" + solicitudeBalconId,
      });
    } else {
      setTimeout(printSolicitudeBalcon, 1000);
    }
  };
  ...
  const excelSolicitude = () => {
    if (Router.asPath !== Router.route) {
      ...
      Router.push({ pathname: "/solicitudeBalcon/excel/" + solicitudeId });
    } else {
  ...
  const txtBGR = () => {
    if (Router.asPath !== Router.route) {
      const solicitudeId = Router.query.id as string;
      Router.push({ pathname: "/solicitudeBalcon/txtBGR/" + solicitudeId });
    } else {
  ...
  useEffect(() => {
    const newFilteredItems = items.filter((item) => {
      return (
        ...
        item.centerCostBalcon.name.toLowerCase().includes(searchTerm) ||
        item.providerBalcon.name.toLowerCase().includes(searchTerm) ||
        item.providerBalcon.emailBalcon.toLowerCase().includes(searchTerm) ||
  ...
  return (
    <>
      <title>Balcón | Editar solicitud</title>
                          ...
                          return (
                            ...
                            >
                              ...
                              {CheckPermissions(auth, [0, 1, 2, 9, 14]) && (
                                <td className="p-1 border border-black">
                                  {getHighlightedText(
                                    item.providerBalcon.emailBalcon,
                                    searchTerm
                                  )}
                                </td>
                              )}
