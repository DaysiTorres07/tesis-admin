...
import StatusAdvanceIg from "../status/[id]";
...
const EditFactureAdvance = () => {
  ...
  const [initialValues, setInitialValues] = useState<AdvanceInmogestion>({
    number: 0,
    soliciter: "",
    date: FormatedDate(),
    details: "",
  ...
  const onSubmit = async (formData: AdvanceInmogestion) => {
    if (Router.asPath !== Router.route) {
      setLoading(true);
      const advanceInmogestionId = Router.query.id as string;
      const factureAdvanceInmogestionItems = await UploadSolicitudeImages(
        items
      );
      const requestData = {
        ...formData,
        itemsComment,
        items: factureAdvanceInmogestionItems,
        id: advanceInmogestionId,
      };
      const response: ResponseData = await HttpClient(
  ...
  const onClickFi = (data: any) => {
    fetch("/api/mailIG/mailAdvaFinan", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const onClickTes = (data: any) => {
    fetch("/api/mailIG/mailAdvaTesor", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const printAdvanceInmogestion = () => {
    if (Router.asPath !== Router.route) {
      const advanceInmogestionId = Router.query.id as string;
      Router.push({
        pathname: "/advanceInmogestion/print/" + advanceInmogestionId,
      });
    } else {
      setTimeout(printAdvanceInmogestion, 1000);
    }
  };
  ...
  const excelAdvance = () => {
    if (Router.asPath !== Router.route) {
      ...
      Router.push({ pathname: "/advanceInmogestion/excel/" + advanceId });
    } else {
  ...
  const txtBGR = () => {
    if (Router.asPath !== Router.route) {
      const advanceId = Router.query.id as string;
      Router.push({ pathname: "/advanceInmogestion/txtBGR/" + advanceId });
    } else {
  ...
  const buttons = {
    ...
    show: (rowData: FactureAdvanceInmogestion) => {
      if (
  ...
  useEffect(() => {
    const newFilteredItems = items.filter((item) => {
      return (
        ...
        item.centerCostIg.name.toLowerCase().includes(searchTerm) ||
        item.providerIg.name.toLowerCase().includes(searchTerm) ||
        item.providerIg.emailIg.toLowerCase().includes(searchTerm) ||
  ...
  return (
    <>
      <title>IG | Editar Anticipo</title>
      <style>
        {`
        .highlight {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 m-4 gap-4 mb-4">
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    disabled={!CheckPermissions(auth, [0, 8])}
                  />
                </div>
                <div>
                  <button
                      ...
                      <div className="relative right-0 w-40 mt-2 origin-top-right bg-white divide-y divide-gray-100 rounded-md shadow-lg ring-1 ring-black ring-opacity-5">
                        {CheckPermissions(auth, [0, 8, 2, 3, 9]) && (
                          <button
                            ...
                            onClick={printAdvanceInmogestion}
                          >
                            PDF
                          </button>
                        )}
                  ...
                  <table className="text-xs w-full">
                    <thead>
                      <tr className="text-center border border-black">
                        <th>Acciones</th>
                        {CheckPermissions(auth, [0, 8, 2, 3, 4, 5, 6, 9]) && (
                          <th>Proyecto</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 2, 3, 4, 5, 9]) && (
                          <th>Centro Costos</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 2, 3, 5, 4, 6, 9]) && (
                          <th>Proveedor</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 2, 9]) && (
                          <th>Email Prov</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 2, 3, 4, 5, 6, 9]) && (
                          <th>Detalle</th>
                        ...
                        {CheckPermissions(auth, [0, 8, 2, 3, 4, 6, 9]) && (
                          <th>Beneficiario</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 2, 3, 4, 6, 9]) && (
                          <th>Cedula o RUC</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 2, 4, 9]) && (
                          <th>Banco Beneficiario</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 2, 4, 9]) && (
                          <th>Numero Cuenta</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 9]) && (
                          <th>Tipo Cuenta</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 9]) && (
                        ...
                        {CheckPermissions(auth, [0, 8, 2, 4, 5, 9]) && (
                          <th>Fecha factura</th>
                        )}
                        {CheckPermissions(auth, [0, 8, 2, 3, 4, 5, 6, 9]) && (
