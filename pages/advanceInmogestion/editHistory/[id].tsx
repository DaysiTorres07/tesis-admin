...
const EditFactureAdvance = () => {
  ...
  const printAdvance = () => {
    if (Router.asPath !== Router.route) {
      ...
      Router.push({ pathname: "/advanceInmogestion/print/" + advanceId });
    } else {
  ...
  const txtPichincha = () => {
    if (Router.asPath !== Router.route) {
      const advanceId = Router.query.id as string;
      Router.push({ pathname: "/advanceInmogestion/txt/" + advanceId });
    } else {
  ...
  return (
    <>
      <title>IG | Historial de Anticipo</title>
              ...
              <div className="grid grid-cols-1 m-4">
                ...
                <div className="w-full mt-3 p-1">
                  <p style={{ marginTop: "10px" }}>
                    ...
                    {items
                      .reduce(
                        (partialSum, factureAdvanceIg) =>
                          partialSum + factureAdvanceIg.valueNet,
                        0
                      )
                      .toLocaleString()}
                  </p>
                  <p>
                    ...
                    {items
                      .reduce(
                        (partialSum, factureAdvanceIg) =>
                          partialSum + factureAdvanceIg.accreditedPayment,
                        0
                      )
                      .toLocaleString()}
                  </p>
                  <p className="text-red-500">
                    ...
                    {items
                      .reduce(
                        (partialSum, factureAdvanceIg) =>
                          partialSum + factureAdvanceIg.difference,
                        0
                      )
                      .toLocaleString()}
                  </p>
