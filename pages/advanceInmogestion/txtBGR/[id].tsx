...
const SoliTxtBGR = () => {
  ...
  const [advanceIg, setAdvanceIg] = useState<AdvanceInmogestion>(null);
  ...
  const loadData = async () => {
    if (Router.asPath !== Router.route) {
      ...
      const advanceIg: AdvanceInmogestion = response.data;
      const facturesAdvIg: Map<String, FactureAdvanceInmogestion> = new Map();
      advanceIg.items.forEach((factureAdvIg: FactureAdvanceInmogestion) => {
        if (
          Array.from(facturesAdvIg.keys()).includes(factureAdvIg.beneficiary)
        ) {
          const auxFact = facturesAdvIg.get(factureAdvIg.beneficiary);
          auxFact.valueNet += factureAdvIg.valueNet;
          facturesAdvIg.set(factureAdvIg.beneficiary, auxFact);
        } else {
          facturesAdvIg.set(factureAdvIg.beneficiary, factureAdvIg);
        }
      });
      setAdvanceIg({
        ...advanceIg,
        items: Array.from(facturesAdvIg.values()),
      });
      setLoading(false);
    } else {
  ...
  return (
    <>
      <title>IG | Cash BGR de Anticipo</title>
      {advanceIg === null ? (
        ...
        <div className="bg-white md:w-2/3 w-11/12 mx-auto p-5">
          ...
          <div className="mx-5">
            <table className="mb-5 border border-black">
              {(advanceIg.items ?? []).map((item, index) => {
