...
const ExcelAdvanceIg = () => {
  const { auth } = useAuth();
  const [loading, setLoading] = useState(true);
  const [advance, setAdvance] = useState<AdvanceInmogestion>(null);
  ...
  const loadData = async () => {
    if (Router.asPath !== Router.route) {
      ...
      const response: ResponseData = await HttpClient(
        "/api/advanceInmogestion/" + advanceId,
        "GET",
        auth.userName,
        auth.role
      );
  ...
  function exportToExcel() {
    ...
    downloadLink.download = "Anticipo-IC.xls";
    downloadLink.click();
      ...
      <LoadingContainer visible={loading}>
        {advance === null ? (
          ...
          <div>
            ...
            <table width="100%" className="mt-5" style={{ fontSize: "12px" }}>
              <thead>
                <tr>
                  <td>
              ...
              <thead>
                <tr>
                  ...
                  <td>
                    ...
                    {advance.items
                      .reduce(
                        (partialSum, facure) => partialSum + facure.difference,
                        0
                      )
                ...
                <tbody>
                  {advance.items.map((item, index) => (
                    <tr key={index} style={{ textAlign: "center" }}>
                      <td className="border border-black">{item.project?.name ?? ""}</td>
                      <td className="border border-black">{item.centerCostIg?.name ?? ""}</td>
                      <td className="border border-black">{item.providerIg?.name ?? ""}</td>
                      <td className="border border-black">{item.factureDate ?? ""}</td>
                      <td className="border border-black">{item.factureNumber ?? ""}</td>
                      <td className="border border-black">{item.details ?? ""}</td>
                      <td className="border border-black">{item.beneficiary ?? ""}</td>
                      <td className="border border-black">{item.identificationCard ?? ""}</td>
                      <td className="border border-black">{item.bank ?? ""}</td>
                      <td className="border border-black">{item.accountBank ?? ""}</td>
                      <td className="border border-black">{(item.value ?? 0).toLocaleString("en-US")}</td>
                      <td className="border border-black">
                        {(item.valueRetention ?? 0).toLocaleString("en-US")}
                      </td>
                      <td className="border border-black">{(item.valueNet ?? 0).toLocaleString("en-US")}</td>
                    </tr>
                  ))}
                  <tr style={{ textAlign: "center" }}>
