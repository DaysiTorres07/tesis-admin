...
interface Props extends ModalProps<AdvanceInmogestion> {
  initialData?: AdvanceInmogestion;
}

const StatusAdvanceIg = (props: Props) => {
  const { auth } = useAuth();
  const [initialValues, setInitialValues] = useState<AdvanceInmogestion>(null);

  const loadData = async () => {
    const solicitudeId = Router.query.id as string;
    const response: ResponseData = await HttpClient(
      "/api/advanceInmogestion/" + solicitudeId,
      "GET",
      auth.userName,
      auth.role
    );
...
export default StatusAdvanceIg;
