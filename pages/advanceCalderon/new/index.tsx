...
import { AdvanceCalderon, FactureAdvanceCalderon, ResponseData } from "../../../lib/types";
...
const NewFactureAdvance = () => {
  ...
  const [initialValues, _setInitialValues] = useState<AdvanceCalderon>({
    number: 0,
  ...
  const formik = useFormik<AdvanceCalderon>({
    ...
    initialValues,
    onSubmit: async (formData: AdvanceCalderon) => {
      if (formData.details.trim() === "") {
        toast.warning("El campo detalle no puede estar vacío");
        return;
      }
      setLoading(true);
      const facutureAdvanceCalderonItems = await UploadSolicitudeImages(items);
      const response: ResponseData = await HttpClient(
        "/api/advanceCalderon",
        "POST",
        auth.userName,
        auth.role,
        { ...formData, items: facutureAdvanceCalderonItems }
      );
      if (response.success) {
  ...
  const buttons = {
    ...
    delete: (rowData: FactureAdvanceCalderon) => showConfirmModal(rowData.id),
  };

  return (
    <RoleLayout permissions={[0, 1, 14]}>
      <title>Calderon | Crear Anticipo</title>
