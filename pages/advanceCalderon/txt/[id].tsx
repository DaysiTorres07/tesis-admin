...
const AdvaTxtPichincha = () => {
  const { auth } = useAuth();
  const [loading, setLoading] = useState(true);
  const [advanceC, setAdvanceC] = useState<AdvanceCalderon>(null);
  ...
  const loadData = async () => {
    if (Router.asPath !== Router.route) {
      ...
      const advanceC: AdvanceCalderon = response.data;
      const facturesAdvC: Map<String, FactureAdvanceCalderon> = new Map();
      advanceC.items.forEach((factureAdvC: FactureAdvanceCalderon) => {
        if (Array.from(facturesAdvC.keys()).includes(factureAdvC.beneficiary)) {
          const auxFact = facturesAdvC.get(factureAdvC.beneficiary);
          auxFact.valueNet += factureAdvC.valueNet;
          facturesAdvC.set(factureAdvC.beneficiary, auxFact);
        } else {
          facturesAdvC.set(factureAdvC.beneficiary, factureAdvC);
        }
      });
      setAdvanceC({
        ...advanceC,
        items: Array.from(facturesAdvC.values()),
      });
      setLoading(false);
    } else {
  ...
  return (
    <>
      <title>Calderon | Cash Pichincha de Anticipo</title>
...
export default AdvaTxtPichincha;
