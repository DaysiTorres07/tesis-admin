...
import { AdvanceCalderon, ModalProps, ResponseData } from "../../../lib/types";
import HttpClient from "../../../lib/utils/http_client";
import theme from "../../../lib/styles/theme";

interface Props extends ModalProps<AdvanceCalderon> {
  initialData?: AdvanceCalderon;
}

const StatusAdvanceC = (props: Props) => {
  const { auth } = useAuth();
  const [initialValues, setInitialValues] = useState<AdvanceCalderon>(null);

  const loadData = async () => {
    const solicitudeId = Router.query.id as string;
    const response: ResponseData = await HttpClient(
      "/api/advanceCalderon/" + solicitudeId,
      "GET",
      auth.userName,
      auth.role
    );
