...
const ExcelSolicitudeIG = () => {
  const { auth } = useAuth();
  const [loading, setLoading] = useState(true);
  const [solicitude, setSolicitude] = useState<SolicitudeInmogestion>(null);
  ...
  const loadData = async () => {
    if (Router.asPath !== Router.route) {
      ...
      const response: ResponseData = await HttpClient(
        "/api/solicitudeInmogestion/" + solicitudeId,
        "GET",
        auth.userName,
        auth.role
      );
  ...
  function exportToExcel() {
    ...
    downloadLink.download = "Solicitud-IG.xls";
    downloadLink.click();
  ...
  return (
    <>
      <title>IG | Excel Solicitud</title>
                ...
                <tbody>
                  {solicitude.items.map((item, index) => (
                    <tr style={{ textAlign: "center" }} key={index}>
                      ...
                      <td className="border border-black">{item.centerCost?.name ?? ""}</td>
                      <td className="border border-black">{item.provider?.name ?? ""}</td>
...
export default ExcelSolicitudeIG;
