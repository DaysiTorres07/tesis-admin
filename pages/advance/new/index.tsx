...
import { ResponseData, FactureAdvance, Advance } from "../../../lib/types";
...
const NewFactureAdvance = () => {
  ...
  const [initialValues, _setInitialValues] = useState<Advance>({
    number: 0,
  ...
  const formik = useFormik<Advance>({
    ...
    onSubmit: async (formData: Advance) => {
      ...
      setLoading(true);
      const facutureAdvanceItems = await UploadSolicitudeImages(items);
      const response: ResponseData = await HttpClient(
        "/api/advance",
        "POST",
        auth.userName,
        auth.role,
        { ...formData, items: facutureAdvanceItems }
      );
      if (response.success) {
  ...
  const buttons = {
    ...
    delete: (rowData: FactureAdvance) => showConfirmModal(rowData.id),
  };

  return (
    <RoleLayout permissions={[0, 1, 14]}>
      <title>IC | Crear Anticipo</title>
