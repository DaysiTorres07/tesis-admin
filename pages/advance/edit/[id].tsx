...
import StatusAdvance from "../status/[id]";
...
const EditFactureAdvance = () => {
  ...
  const [items, setItems] = useState<Array<FactureAdvance>>([]);
  const [initialValues, setInitialValues] = useState<Advance>({
    number: 0,
    soliciter: "",
    date: FormatedDate(),
    details: "",
  ...
  const [editingFactureAdvance, setEditingFactureAdvance] =
    useState<FactureAdvance | null>(null);
  ...
  const onSubmit = async (formData: Advance) => {
    if (Router.asPath !== Router.route) {
      setLoading(true);
      const advanceId = Router.query.id as string;
      const factureAdvanceItems = await UploadSolicitudeImages(items);
      const requestData = {
        ...formData,
        itemsComment,
        items: factureAdvanceItems,
        id: advanceId,
      };
      const response: ResponseData = await HttpClient(
        "/api/advance",
  ...
  const onClickFi = (data: any) => {
    fetch("/api/mailIC/mailAdvaFinan", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const onClickTes = (data: any) => {
    fetch("/api/mailIC/mailAdvaTesor", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const printAdvance = () => {
    if (Router.asPath !== Router.route) {
      const advanceId = Router.query.id as string;
      Router.push({ pathname: "/advance/print/" + advanceId });
    } else {
  ...
  const excelAdvance = () => {
    if (Router.asPath !== Router.route) {
      const advanceId = Router.query.id as string;
      Router.push({ pathname: "/advance/excel/" + advanceId });
    } else {
  ...
  const txtPichincha = () => {
    if (Router.asPath !== Router.route) {
      const advanceId = Router.query.id as string;
      Router.push({ pathname: "/advance/txt/" + advanceId });
    } else {
  ...
  const txtBGR = () => {
    if (Router.asPath !== Router.route) {
      const advanceId = Router.query.id as string;
      Router.push({ pathname: "/advance/txtBGR/" + advanceId });
    } else {
  ...
  const buttons = {
    ...
    delete: (rowData: FactureAdvance) => {
      if (CheckPermissions(auth, [0, 9])) {
        showConfirmModal(rowData.id);
      }
    },

    show: (rowData: FactureAdvance) => {
      if (
  ...
  useEffect(() => {
    const newFilteredItems = items.filter((item) => {
      return (
        ...
        item.centerCost.name.toLowerCase().includes(searchTerm) ||
        item.provider.name.toLowerCase().includes(searchTerm) ||
        item.provider.email.toLowerCase().includes(searchTerm) ||
  ...
  return (
    <>
      <title>IC | Editar ancitipo</title>
      <style>
        {`
        .highlight {
      ...
      <div className="flex h-full">
        ...
        <div className="w-12/12 bg-white my-14 mx-8">
            ...
            <div className="grid md:grid-cols-3 flex justify-center items-center my-4 text-center">  
              <label className="text-gray-700 text-lg font-family: ui-sans-serif font-bold mx-4">
                Anticipo #{" "}
                <em
                  style={{
                    color: "#bb22dd",
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 flex h-full m-4 gap-4 mb-4 text-center">
              ...
              <div className="grid grid-cols-1 gap-4 m-2">
                  ...
                  <table className="text-xs w-full">
                    <thead>
                      <tr className="text-center border border-black">
                        <th>Acciones</th>
                        {CheckPermissions(auth, [0, 1, 2, 3, 4, 5, 6, 9, 14]) && (
                          <th>Proyecto</th>
                        )}
                        {CheckPermissions(auth, [0, 1, 2, 3, 4, 5, 9, 14]) && (
                          <th>Centro Costos</th>
                        )}
                        {CheckPermissions(auth, [0, 1, 2, 3, 5, 4, 6, 9, 14]) && (
                          <th>Proveedor</th>
                        )}
                ...
                <div className="mx-16 my-3 py-2 px-8 text-left border rounded-lg bg-slate-100">
                  <p className="mt-1 font-family: ui-sans-serif">
                  ...
                  <p className="mt-1 text-red-500 font-family: ui-sans-serif">
                ...
                >
                  ...
                  <p className="my-4 font-family: ui-sans-serif text-center">               
              <em
                style={{
                  color: "#334155",
                  fontStyle: "normal",
              ...
              <FactureAdvanceModal
                ...
                onDone={(newItem: FactureAdvance) => {
                  if (editingFactureAdvance === null) {
                    ...
                    setItems((oldData) =>
                      oldData.map((element: FactureAdvance) =>
              ...
              <ConfirmModal
                ...
                onDone={() => {
                  setItems((oldData) => [
                    ...oldData.filter(
                      (item: FactureAdvance) => item.id !== itemToDelete
                    ),
                  ]);
                  hideConfirmModal();
                }}
