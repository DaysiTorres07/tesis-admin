...
import FactureCalderonModal from "../../../lib/components/modals/factureCalderon";
import { useForm } from "react-hook-form";
import StatusSolicitudeCalderon from "../status/[id]";
...
const EditFacture = () => {
  ...
  const [items, setItems] = useState<Array<FactureCalderon>>([]);
  const [initialValues, setInitialValues] = useState<SolicitudeCalderon>({
    number: 0,
    soliciter: "",
    date: FormatedDate(),
    details: "",
  ...
  const [editingFacture, setEditingFacture] = useState<FactureCalderon | null>(
    null
  );
  ...
  const onSubmit = async (formData: SolicitudeCalderon) => {
    if (Router.asPath !== Router.route) {
      setLoading(true);
      const solicitudeCalderonId = Router.query.id as string;
      const factureCalderonItems = await UploadSolicitudeImages(items);
      const requestData = {
        ...formData,
        itemsComment,
        items: factureCalderonItems,
        id: solicitudeCalderonId,
      };
      const response: ResponseData = await HttpClient(
  ...
  const onClickSo = (data: any) => {
    fetch("/api/mailCalderon/mailSoli", {
      method: "POST",
      headers: {
  ...
  const onClickCo = (data: any) => {
    fetch("/api/mailCalderon/mailConta", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const onClickTe = (data: any) => {
    fetch("/api/mailCalderon/mailteso", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const onClickFi = (data: any) => {
    fetch("/api/mailCalderon/mailFinan", {
      method: "POST",
      headers: {
        Accept: "application/json, text/plain, */*",
        "Content-Type": "application/json",
  ...
  const formik = useFormik<SolicitudeCalderon>({
  ...
  const printSolicitudeCalderon = () => {
    if (Router.asPath !== Router.route) {
      const solicitudeCalderonId = Router.query.id as string;
      Router.push({
        pathname: "/solicitudeCalderon/print/" + solicitudeCalderonId,
      });
    } else {
      setTimeout(printSolicitudeCalderon, 1000);
    }
  };
  ...
  const excelSolicitude = () => {
    if (Router.asPath !== Router.route) {
      ...
      Router.push({ pathname: "/solicitudeCalderon/excel/" + solicitudeId });
    } else {
  ...
  const txtPichincha = () => {
    if (Router.asPath !== Router.route) {
      const solicitudeCalderonId = Router.query.id as string;
      Router.push({
        pathname: "/solicitudeCalderon/txt/" + solicitudeCalderonId,
      });
    } else {
  ...
  const columns: ColumnData[] = [
    ...
    {
      dataField: "providerCalderon.emailCalderon",
      caption: "Email Prov",
      cssClass: "column-soli",
    },
    {
  ...
  const buttons = {
    ...
    delete: (rowData: FactureCalderon) => {
      if (CheckPermissions(auth, [0, 9])) {
        showConfirmModal(rowData.id);
      }
    },

    show: (rowData: FactureCalderon) => {
      if (
  ...
  useEffect(() => {
    const newFilteredItems = items.filter((item) => {
      return (
        ...
        item.observationConta.toLowerCase().includes(searchTerm) ||
  ...
  return (
    <>
      <title>CALDERON | Editar Solicitud</title>
        ...
        <div className="w-12/12 md:w-5/6 flex items-center justify-center">
          <div className="w-12/12 bg-white my-14 mx-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 flex h-full m-4 gap-4 mb-4 text-center">
                      ...
                      <div className="relative text-center w-40 mx-14  bg-white divide-y divide-gray-100 shadow-lg ring-1 ring-black ring-opacity-5">
                        {CheckPermissions(auth, [0, 1, 2, 3, 9, 14]) && (
                          <button
                            ...
                            onClick={printSolicitudeCalderon}
                          >
                            PDF
                          </button>
                        )}
              ...
              <FactureCalderonModal
                visible={modalVisible}
                close={hideModal}
                initialData={editingFacture}
                onDone={(newItem: FactureCalderon) => {
                  if (editingFacture === null) {
                    ...
                    setItems((oldData) =>
                      oldData.map((element: FactureCalderon) =>
              ...
              <ConfirmModal
                ...
                onDone={() => {
                  setItems((oldData) => [
                    ...oldData.filter(
                      (item: FactureCalderon) => item.id !== itemToDelete
                    ),
                  ]);
                  hideConfirmModal();
                }}
