...
import { FactureCalderon, ResponseData, SolicitudeCalderon } from "../../../lib/types";
...
const NewFacture = () => {
  ...
  const [initialValues, _setInitialValues] = useState<SolicitudeCalderon>({
    number: 0,
  ...
  const formik = useFormik<SolicitudeCalderon>({
    ...
    initialValues,
    onSubmit: async (formData: SolicitudeCalderon) => {
      if (formData.details.trim() === "") {
        toast.warning("El campo detalle no puede estar vacío");
        return;
      }
      setLoading(true);
      const facutureCalderonItems = await UploadSolicitudeImages(items);
      const response: ResponseData = await HttpClient(
        "/api/solicitudeCalderon",
        "POST",
        auth.userName,
        auth.role,
        { ...formData, items: facutureCalderonItems }
      );
      if (response.success) {
  ...
  const buttons = {
    ...
    delete: (rowData: FactureCalderon) => showConfirmModal(rowData.id),
  };

  return (
    <RoleLayout permissions={[0, 1, 14]}>
      <title>Calderon | Crear Solicitud de Pagos</title>
