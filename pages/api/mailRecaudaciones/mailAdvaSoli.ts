...
export default async function (req: { body: { number: any; soliciter: any; }; }, res: { status: (arg0: number) => { (): any; new(): any; json: { (arg0: { status: string; }): void; new(): any; }; }; }) {
  ...
  const mailData = {
    ...
    <h4>DETALLE:</h4>
    <p>Se ingreso el Anticipo N°: <strong>${req.body.number}</strong>, de la empresa <strong>RECAUDACIONES</strong>, por el Usuario: <strong>${req.body.soliciter}</strong>, realizar las tareas correspondientes.</p>
