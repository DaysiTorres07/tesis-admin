...
export default async function (req: { body: { number: any; }; }, res: { status: (arg0: number) => { (): any; new(): any; json: { (arg0: { status: string; }): void; new(): any; }; }; }) {
  ...
  const mailData = {
    ...
    html: `
          ...
          <h4>DETALLE:</h4>
          <p>El financiero culmino su tarea de la Solicitud N°: <strong>${req.body.number}</strong>, de la empresa <strong>BALCON DEL CAONY</strong>, por el Usuario: <strong>Daniela</strong>, realizar las tareas correspondientes.</p>
