...
// Inicio de la app
const AdvanceICHistoryPanel = (props: Props) => {
  const { auth } = useAuth();
  ...
  const loadData = async () => {
    ...
    if (
      ...
      var response = await HttpClient(
        "/api/advance/advanceHistory?dates=" +
          props.dates[0] +
          "¡" +
          props.dates[1],
        "GET",
    ...
    const solicitudes: Array<Advance> = response.data ?? [];
  ...
  const buttons = {
    ...
    edit: (rowData: FactureAdvance) =>
      !CheckPermissions(auth, [8])
        ? Router.push({
            pathname: "/advance/editHistory/" + (rowData.id as string),
          })
  ...
  return (
    <>
      {/* <h3 className="text-center my-4 mb-4 text-2xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">
        HISTORIAL DE ANTICIPOS INMOCONSTRUCCIONES
      </h3> */}
...
export default AdvanceICHistoryPanel;
