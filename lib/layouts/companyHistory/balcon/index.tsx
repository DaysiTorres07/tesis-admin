...
// Inicio de la app
const SolicitudeBalconHistoryPanel = (props: Props) => {
  const { auth } = useAuth();
  ...
  const loadData = async () => {
    ...
    if (
      ...
      var response = await HttpClient(
        "/api/solicitudeBalcon/solicitudeHistory",
        "GET",
        auth.userName,
        auth.role
      );
      ...
      var response = await HttpClient(
        "/api/solicitudeBalcon/solicitudeHistory?dates=" +
          props.dates[0] +
          "¡" +
          props.dates[1],
        "GET",
  ...
  const buttons = {
    ...
    edit: (rowData: FactureBalcon) =>
      !CheckPermissions(auth, [8])
        ? Router.push({
            pathname: "/solicitudeBalcon/editHistory/" + (rowData.id as string),
          })
  ...
  return (
    <>
      {/* <h1 className="text-center my-4 mb-4 text-2xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">
        HISTORIAL DE SOLICITUDES BALCON DEL CAONY
      </h1> */}
...
export default SolicitudeBalconHistoryPanel;
