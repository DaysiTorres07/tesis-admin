...
// Inicio de la app
const SolicitudeIGHistoryPanel = (props: Props) => {
  const { auth } = useAuth();
  ...
  const loadData = async () => {
    ...
    if (
      ...
      var response = await HttpClient(
        "/api/solicitudeInmogestion/solicitudeHistory",
        "GET",
        auth.userName,
        auth.role
      );
      ...
      var response = await HttpClient(
        "/api/solicitudeInmogestion/solicitudeHistory?dates=" +
          props.dates[0] +
          "¡" +
          props.dates[1],
        "GET",
  ...
  const buttons = {
    ...
    edit: (rowData: FactureInmogestion) =>
      !CheckPermissions(auth, [8])
        ? Router.push({
            pathname:
              "/solicitudeInmogestion/editHistory/" + (rowData.id as string),
          })
  ...
  return (
    <>
      {/* <h3 className="text-center my-4 mb-4 text-2xl font-extrabold leading-none tracking-tight text-gray-900 md:text-3xl lg:text-4xl dark:text-white">
        HISTORIAL DE SOLICITUDES INMOGESTIÓN
      </h3> */}
...
export default SolicitudeIGHistoryPanel;
