...
const CentersBalconPanel = () => {
  const { auth } = useAuth();
  const [loading, setLoading] = useState<boolean>(true);
  const [tableData, setTableData] = useState<Array<FactureCenterBalcon>>([]);
  ...
  const loadData = async () => {
    ...
    const centersBalcon: Array<FactureCenterBalcon> = response.data ?? [];
    setTableData(centersBalcon);
    setLoading(false);
  };
  ...
  const removeRow = async (data: any) => {
    const response = await HttpClient(
      "/api/centro_costosBalcon/" + data.id,
      "DELETE",
      auth.userName,
      auth.role
    );
...
export default CentersBalconPanel;
