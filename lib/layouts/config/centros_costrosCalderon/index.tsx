...
const CentersCalderonPanel = () => {
  const { auth } = useAuth();
  const [loading, setLoading] = useState<boolean>(true);
  const [tableData, setTableData] = useState<Array<FactureCenterCalderon>>([]);
  ...
  const loadData = async () => {
    ...
    const centersCalderon: Array<FactureCenterCalderon> = response.data ?? [];
    setTableData(centersCalderon);
    setLoading(false);
  };
  ...
  const removeRow = async (data: any) => {
    const response = await HttpClient(
      "/api/centro_costosCalderon/" + data.id,
      "DELETE",
      auth.userName,
      auth.role
    );
...
export default CentersCalderonPanel;
