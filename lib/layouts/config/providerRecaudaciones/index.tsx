...
const ProvidersRecaudacionesPanel = () => {
  const { auth } = useAuth();
  const [loading, setLoading] = useState<boolean>(true);
  const [tableData, setTableData] = useState<Array<FactureProviderRecaudaciones>>([]);
  ...
  const loadData = async () => {
    ...
    const providersRecaudaciones: Array<FactureProviderRecaudaciones> = response.data ?? [];
    setTableData(providersRecaudaciones);
    setLoading(false);
  };
  ...
  const removeRow = async (data: any) => {
    const response = await HttpClient(
      "/api/providerRecaudaciones/" + data.id,
      "DELETE",
      auth.userName,
      auth.role
    );
...
export default ProvidersRecaudacionesPanel;
