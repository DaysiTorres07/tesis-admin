...
const ProvidersCalderonPanel = () => {
  const { auth } = useAuth();
  const [loading, setLoading] = useState<boolean>(true);
  const [tableData, setTableData] = useState<Array<FactureProviderCalderon>>([]);
  ...
  const loadData = async () => {
    ...
    const providersCalderon: Array<FactureProviderCalderon> = response.data ?? [];
    setTableData(providersCalderon);
    setLoading(false);
  };
...
export default ProvidersCalderonPanel;
