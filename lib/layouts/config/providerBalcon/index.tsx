...
const ProvidersBalconPanel = () => {
  const { auth } = useAuth();
  const [loading, setLoading] = useState<boolean>(true);
  const [tableData, setTableData] = useState<Array<FactureProviderBalcon>>([]);
  ...
  const loadData = async () => {
    ...
    const providersBalcon: Array<FactureProviderBalcon> = response.data ?? [];
    setTableData(providersBalcon);
    setLoading(false);
  };
...
export default ProvidersBalconPanel;
