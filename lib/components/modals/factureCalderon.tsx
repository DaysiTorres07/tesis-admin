...
import { FactureCalderon, ModalProps, ResponseData } from "../../types";
import { CheckPermissions } from "../../utils/check_permissions";
import HttpClient from "../../utils/http_client";

const initialFacture: FactureCalderon = {
  id: null,
  //Solicitante
  project: {
    name: "CALDERON",
...
interface Props extends ModalProps<FactureCalderon> {
  initialData?: FactureCalderon;
}

const FactureCalderonModal = (props: Props) => {
  const { auth } = useAuth();
  const [initialValues, setInitialValues] =
    useState<FactureCalderon>(initialFacture);
  ...
  const formik = useFormik<FactureCalderon>({
    enableReinitialize: true,
    validateOnMount: true,
    validateOnBlur: true,
    validateOnChange: true,
    initialValues,
    onSubmit: async (formData: FactureCalderon) => {
      ...
      if (formData.providerCalderon.name === "") {
        ...
        <div className="bg-white p-6 rounded shadow-lg z-10 w-2/3 h-5/6 overflow-y-auto">
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-4 mb-3">
              ...
              <div>
                {CheckPermissions(auth, [0, 3, 1, 8, 4]) && (
                  <div>
                    <label className="text-gray-700 text-sm font-bold mb-2">
                     * Beneficiario
                    </label>
...
export default FactureCalderonModal;
