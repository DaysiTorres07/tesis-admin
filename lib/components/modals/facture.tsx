...
const FactureModal = (props: Props) => {
  ...
  const [selectedProject, setSelectedProject] = useState<FactureProject | null>(
    null
  );
  ...
  // función para actualizar el proyecto y los centros de costos filtrados
  const handleProjectSelection = (projectName: string) => {
    ...
    setFilteredCenters(
      centers.filter((center) => {
        const project = projects.find((p) => p.id === center.projectId);
        return project?.name === selectedProject?.name;
      })
    );
    formik.setFieldValue("project.name", selectedProject?.name);
  };

  const handleCenterCostFilter = (event) => {
    const value = event.target.value;
    const filteredCenters = centers.filter(
      (center) =>
        center.projectId === (selectedProject?.id ?? "") &&
  ...
  const validationSchema = Yup.object({
    centerCost: Yup.object({
      name: Yup.string()
        .required("El centro de costos es obligatorio")
        .matches(
          /^[\d\.]+ - .+$/,
          "Ingrese el centro de costos completo (ej: 1.01 - ML TERRENO)"
        ),
    }),
  });
  ...
  const formik = useFormik<Facture>({
    ...
    onSubmit: async (formData: Facture) => {
      ...
      if (formData.project.name === "") {
        toast.warning("El Proyecto no puede estar vacio");
        return;
      }
      if (formData.centerCost.name === "") {
              ...
              <div>
                {CheckPermissions(auth, [0, 1]) && (
                  <>
                    ...
                    >
                      <option>Seleccione un projecto</option>
                ...
                {CheckPermissions(auth, [0, 1]) && (
                  <>
                    ...
                    <input
                      ...
                      onKeyUp={handleCenterCostFilter}
                      list="centerCosts"
                      disabled={!selectedProject}
                      autoComplete="off"
                    />
                    <datalist id="centerCosts">
