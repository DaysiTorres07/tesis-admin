...
import { FactureInmogestion, ModalProps, ResponseData } from "../../types";
import { CheckPermissions } from "../../utils/check_permissions";
import HttpClient from "../../utils/http_client";

const initialFacture: FactureInmogestion = {
  id: null,
  //Solicitante
  project: {
    name: "IG",
...
interface Props extends ModalProps<FactureInmogestion> {
  initialData?: FactureInmogestion;
}

const FactureIgModal = (props: Props) => {
  const { auth } = useAuth();
  const [initialValues, setInitialValues] =
    useState<FactureInmogestion>(initialFacture);
  ...
  const formik = useFormik<FactureInmogestion>({
    enableReinitialize: true,
    validateOnMount: true,
    validateOnBlur: true,
    validateOnChange: true,
    initialValues,
    onSubmit: async (formData: FactureInmogestion) => {
        ...
        <div className="bg-white p-6 rounded shadow-lg z-10 w-2/3 h-5/6 overflow-y-auto">
          <form onSubmit={formik.handleSubmit}>
            ...
            <div className="grid md:grid-cols-2 grid-cols-1 gap-4 mb-3">
                ...
                {CheckPermissions(auth, [0, 2, 8]) && (
                  <div>
                    ...
                    <input
                      ...
                      value={formik.values?.centerCostIg.name}
                      onChange={(e) =>
                        onChangeHandlerCC(
                          e.target.value,
                          "centerCostIg.name",
                ...
                {CheckPermissions(auth, [0, 8]) && (
                  <>
                    ...
                    <input
                      ...
                      value={formik.values?.providerIg.name}
                      onChange={(e) =>
                        onChangeHandlerP(
                          e.target.value,
                          "providerIg.name",
                    ...
                    {suggestionsP &&
                      suggestionsP.map((suggestion, i) => (
                        ...
                        >
                          {suggestion.name} - {suggestion.emailIg}
                        </div>
                      ))}
                  </>
                )}
                ...
                {CheckPermissions(auth, [0, 8]) && (
                  <>
                    ...
                    <input
                      ...
                      name="providerIg.emailIg"
                      value={formik.values?.providerIg.emailIg}
              ...
              <div>
                {CheckPermissions(auth, [0, 2, 3, 4, 6]) && (
                  <>
                    <label className="text-gray-700 text-sm font-bold mb-2">
                     * Valor a Pagar
                    </label>
              ...
              <div>
                {CheckPermissions(auth, [0, 3, 1, 4, 8]) && (
                  <div>
                    <label className="text-gray-700 text-sm font-bold mb-2">
                     * Beneficiario
                    </label>
